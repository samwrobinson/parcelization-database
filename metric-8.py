...
def save_town_data(df, town_codes, writer):
    for town_code in town_codes:
        ...
        sheet_name = f'{town_name}_metric8'
        df_filtered.to_excel(writer, sheet_name=sheet_name, index=False)

# Function to rename columns for more understandable legends
def rename_columns(df):
    df = df.rename(columns={
        'Metric8_Residential': 'Residential',
        'Metric8_Farm': 'Farm',
        'Metric8_Woodland': 'Woodland',
        'Metric8_Other': 'Other',
        'Metric8_Blanks': 'Blanks',
        'Metric8_BadCat': 'Bad Category'
    })
    return df
...
def generate_town_graphs(df, town_codes):
    ...
    for town_code in town_codes:
        ...
        pivot_table_town = df_filtered.pivot_table(
            index='TaxYear',
            columns='Town',
            values=['Residential', 'Farm', 'Woodland', 'Other', 'Blanks', 'Bad Category'],
            aggfunc='sum'
        )
        
        # Plotting the change over time for the town
        pivot_table_town.plot(kind='line', title=f'Total Acreage by Parcel Type, 2004 - 2020 - {town_name}')
        plt.ylabel('Total Acreage')
        plt.xlabel('Year')
        plt.legend(title='Parcel Types')
        plt.show()
...
def main():
    ...
    sheet_name = 'tbl_Metric8_step2_Town'
    df = pd.read_excel(xls, sheet_name=sheet_name)
    
    # Initialize a new Excel writer object
    output_path = '/Users/MRVPDAir/Desktop/scripts/parcelization_db/Analysis/Parcelization_Analysis_Metric8.xlsx'
