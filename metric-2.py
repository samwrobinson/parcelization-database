...
def save_town_data(df, town_codes, writer):
    for town_code in town_codes:
        ...
        sheet_name = f'{town_name}_metric2'
...
def generate_town_graphs(df, town_codes):
    for town_code in town_codes:
        ...
        pivot_table_town = df_filtered.pivot_table(
            index='TaxYear',
            columns='Town',
            values=['Metric2_0To2Acres', 'Metric2_2To5Acres', 'Metric2_5To10Acres',
                    'Metric2_10To25Acres', 'Metric2_25To50Acres', 'Metric2_50To100Acres',
                    'Metric2_100To200Acres', 'Metric2_GT200Acres'],
            aggfunc='sum'
        )
        
        # Plotting the change over time for the town
        pivot_table_town.plot(kind='line', title=f'Total Acres by Parcel Size, 2004 - 2020 - {town_name}')
...
def main():
    ...
    sheet_name = 'tbl_Metric2_step2_Town'
    df = pd.read_excel(xls, sheet_name=sheet_name)
    
    # Initialize a new Excel writer object
    output_path = '/Users/MRVPDAir/Desktop/scripts/parcelization_db/Analysis/Parcelization_Analysis_Metric2.xlsx'
